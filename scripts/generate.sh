...
if [ $GIT = "true" ] ; then
    echo "RUN apt-get install -y libxss1 software-properties-common build-essential make libssl-dev libghc-zlib-dev libcurl4-gnutls-dev libexpat1-dev gettext unzip"
    echo "RUN wget https://github.com/git/git/archive/v2.30.1.zip -O latestgit.zip"
    echo "RUN unzip latestgit.zip"
    echo "RUN cd git-2.30.1; make prefix=/usr/local all; make prefix=/usr/local install"
